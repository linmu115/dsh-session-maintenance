...
import { hostWorkspaceSyncSchema, type HostWorkspaceSyncRequest, type HostWorkspaceSyncReceipt, type LogicalWorkspaceId } from '@linmu/dsh-session-contracts';
...
export function createHostWorkspaceSync(input: { runtime: HostBarrierRuntime & Record<string, any>; identity: InstanceLeaseIdentity;
  ...
  const apply = async (body: HostWorkspaceSyncRequest): Promise<HostWorkspaceSyncReceipt> => {
    ...
    if (body.projection.sessions.some(item => !selected(item.workspaceId) || item.events.some(event => event.logicalSessionId !== item.session.id)))
  ...
  const handler = async (request: IncomingMessage, response: ServerResponse) => {
    ...
    try {
      if (request.method !== 'POST' || new URL(request.url ?? '/', 'http://localhost').pathname !== HOST_WORKSPACE_SYNC_PATH) return send(404, { code: 'NOT_FOUND' });
      ...
      const body = hostWorkspaceSyncSchema.parse(JSON.parse(Buffer.concat(chunks).toString('utf8'))) as unknown as HostWorkspaceSyncRequest;

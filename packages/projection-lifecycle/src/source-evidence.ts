...
export function sourceWithEvidence(source:CanonicalProjectionSource,adapter:DshSessionAdapterV1,evidence?:AdapterEvidencePort,resolveOwner?:SourceAdapterResolver):CanonicalProjectionSource {
 ...
 return {load:async run=>project(await source.load(run)),...(source.scopeRevision?{scopeRevision:run=>source.scopeRevision!(run)}:{}),...(source.loadVersionEvents?{loadVersionEvents:async(id,version)=>(await restore(await source.loadVersionEvents!(id,version),version)).events}:{}),...(incremental.currentRevision&&incremental.listChanges&&incremental.loadSessions?{currentRevision:()=>incremental.currentRevision!(),listChanges:(input:Parameters<IncrementalCanonicalProjectionSource["listChanges"]>[0])=>incremental.listChanges!(input),loadSessions:async(...args:Parameters<IncrementalCanonicalProjectionSource["loadSessions"]>)=>project(await incremental.loadSessions!(...args))}:{})};
}

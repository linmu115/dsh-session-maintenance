...
async function fixture(officialStorage=false,gpt=false){
 const root=await mkdtemp(join(tmpdir(),"synthetic-v3-binding-")),key=hash(JSON.stringify(["synthetic-instance","web","main",gpt?"dsh-gpt-compat":"dsh-0.1.5",gpt?"dsh-gpt-compat-v1-jsonl-zstd":"dsh-0.1.5-v3-jsonl-zstd-v1"])),nativeRoot=join(root,"native-spaces",key,"sessions");await mkdir(nativeRoot,{recursive:true});await writeFile(join(dirname(nativeRoot),"space.json"),JSON.stringify({schemaVersion:1,key,owner:"synthetic-run",state:"ready",files:{}}));
 const implementations:Record<string,string>={
  ...
  projectionCache:`export const projectionCacheDomainSpec={name:'session_projcache',version:7};export default class {}`,
 ...
 const runtime={sessions:new loaded.session.default(),sessionPersistence:officialContext?.sessionPersistence??new loaded.jsonlPersistence.default(nativeRoot),workspaceRegistry:new loaded.workspace.default(),sessionProjectionCache:new loaded.projectionCache.default(),sessionQuery:new loaded.sessionQuery.default(),storageDomain:{open:async()=>{throw new Error("domain already open")},get:()=>({name:"session_projcache",table:(name:string)=>{if(name!=="sessions")throw new Error("wrong table");return {get:(id:string)=>records.get(id),put:async(id:string,v:unknown)=>{records.set(id,v)},delete:async(id:string)=>{records.delete(id)}}},close:async()=>{throw new Error("borrower must not close host domain")}})}};
...
describe("concrete RC2 Core binding (synthetic modules)",()=>{
 it("refuses an uninitialized host domain before installing persistence guards",async()=>{const f=await fixture();try{f.runtime.storageDomain.get=(()=>undefined) as any;const before=f.runtime.sessionPersistence.open;await expect(createDsh015CoreHostBinding(f.input)).rejects.toThrow("not initialized");expect(f.runtime.sessionPersistence.open).toBe(before);}finally{await rm(f.root,{recursive:true,force:true})}});
...
it("binds the plugin native space from the pinned receipt without replacing ordinary V3",async()=>{
 const f=await fixture(false,true);let binding:Awaited<ReturnType<typeof createDsh015CoreHostBinding>>|undefined;
 try{binding=await createDsh015CoreHostBinding(f.input);expect((await binding.host.observeContract()).platformVersion).toBe("0.1.5-rc.2");}
 finally{await binding?.dispose();await rm(f.root,{recursive:true,force:true});}
});

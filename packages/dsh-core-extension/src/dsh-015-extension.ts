...
import { assessDsh015CoreContract } from "./dsh-015-contract.js";
import type { Dsh015CoreHost } from "./dsh-015-host.js";
import type {
  DshCoreApplyRequest,
...
export class LockedDsh015CoreExtension implements DshCoreExtension {
  readonly host: Dsh015CoreHost;
  private readonly sessionTails = new Map<string, Promise<void>>();

  constructor(host: Dsh015CoreHost, readonly expectedContractFingerprint:string) {
    this.host = host;
  }

  async probe(): Promise<DshCoreProbe> {
    return assessDsh015CoreContract(await this.host.observeContract(),this.expectedContractFingerprint);
  }
  ...
  async capture(request: DshCoreCaptureRequest): Promise<DshCoreSnapshot> {
    ...
    if (session.exists && (session.header.id !== request.sessionId || session.header.version !== 3)) {
  ...
  private async applyLocked(request: DshCoreApplyRequest): Promise<DshCoreState> {
    ...
    if (request.header !== undefined) {
      if (request.header.id !== sessionId || request.header.version !== 3) {
      ...
      await this.host.createSessionWithEvents(request.header, request.events, request.inheritedEventCount ?? 0);
      mutated = true;
  ...
  private async assertCompatible(): Promise<string> {
    ...
    if (
      ...
      probe.contractFingerprint !== this.expectedContractFingerprint
    ) {
      throw new SessionMaintenanceError(
        "ADAPTER_INCOMPATIBLE",
        "DSH Core extension contract does not match official 0.1.5-rc.2",
      );
    }
  ...
  private async withSessionLock<T>(sessionId: string, operation: () => Promise<T>): Promise<T> {
    ...
    await previous;
    try {
      return await this.host.withOfflineSession(sessionId,operation);
    } finally {
      release();

...
import { adapter, alpha2NativeSessionId, alpha2ProjectedNativeRevision } from "../src/index.js";

const at = "2026-09-01T00:00:00.000Z";

describe("Alpha2 projected native revision", () => {
  it("counts expanded native events instead of packed canonical storage rows", async () => {
    ...
    const payload = sessions.get(alpha2NativeSessionId(logicalSessionId))!;

    expect(canonical.events).toHaveLength(1);
    expect(alpha2ProjectedNativeRevision(canonical, payload)).toBe(3);

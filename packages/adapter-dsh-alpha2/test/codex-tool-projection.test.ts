...
import { alpha2NativeSessionId, materializeAlpha2 } from "../src/materialize.js";
...
async function project(events: readonly CanonicalEventV1[]) {
  ...
  return sessions.get(alpha2NativeSessionId(logicalSessionId)) as {
...
describe("Alpha2 Codex tool projection", () => {
  it("materializes a request-valid assistant/message -> tool/call -> tool/result chain", async () => {
    ...
    // Mirrors Alpha2 Session.deriveMessages(): log-only tool/call is skipped,
  ...
  it("keeps an unmatched historical tool result off the Alpha2 message surface", async () => {
    const projected = await project([

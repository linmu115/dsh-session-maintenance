...
import { catalogDigest, digest, type Alpha2ProjectionSession } from "./materialize.js";
...
function parsePayload(value: JsonValue): Alpha2ProjectionSession {
  if (!isRecord(value) || value.schemaVersion !== 1 || !isRecord(value.header) || !Array.isArray(value.events)) {
    throw new TypeError("Alpha2 projection session payload is invalid");
  }
  return value as unknown as Alpha2ProjectionSession;
}
...
export async function inspectAlpha2(reader: ProjectionReader): Promise<ProjectionInspection> {
  ...
  return {
    ...
    issues: [...heldOut].sort().map((type) => ({
      code: "ALPHA2_EVENT_HELD_OUT",
      message: `Event type ${type} was preserved verbatim but is not interpreted by the Alpha2 core codec`,
      sourceType: type,
    })),
  };
}

export function verifyAlpha2(
  ...
  return {
    ...
    issues: ok ? actual.issues : [{
      code: "ALPHA2_PROJECTION_DIGEST_MISMATCH",
      message: "The inspected Alpha2 projection does not match its materialization manifest",
    }, ...actual.issues],
  };
}

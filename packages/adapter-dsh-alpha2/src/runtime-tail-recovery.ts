...
export type Alpha2RuntimeTailRecoveryErrorCode =
...
export class Alpha2RuntimeTailRecoveryError extends Error {
  readonly code: Alpha2RuntimeTailRecoveryErrorCode;

  constructor(code: Alpha2RuntimeTailRecoveryErrorCode, message: string, options?: ErrorOptions) {
    super(message, options);
    this.name = "Alpha2RuntimeTailRecoveryError";
    this.code = code;
  }
}

/**
 * Durable Maintenance watermark for one Alpha2 native session.
 *
 ...
 */
export interface Alpha2CommittedRuntimeSession {
...
export interface Alpha2RuntimeTailRecoveryInput {
...
function fail(
  code: Alpha2RuntimeTailRecoveryErrorCode,
  message: string,
  cause?: unknown,
): never {
  throw new Alpha2RuntimeTailRecoveryError(code, message, cause === undefined ? undefined : { cause });
}
...
function encodeSegment(raw: string): string {
  if (raw.length === 0) fail("RECOVERY_HEADER_MISMATCH", "Alpha2 session ID cannot be empty");
  if (raw === ".") return "~002E";
...
function projectKey(cwd: string): string {
  if (cwd.length === 0) fail("RECOVERY_HEADER_MISMATCH", "Alpha2 SessionHeader.cwd cannot be empty");
  let readable = "";
...
function expectedArtifactPath(root: string, header: JsonRecord, compression: RuntimeArtifact["compression"]): string {
  ...
  const project = cwd === undefined ? "_no-cwd" : typeof cwd === "string" ? projectKey(cwd) : fail(
    "RECOVERY_HEADER_MISMATCH",
    "Alpha2 SessionHeader.cwd must be a string when present",
  );
  const id = header.id;
  if (typeof id !== "string") fail("RECOVERY_HEADER_MISMATCH", "Alpha2 SessionHeader.id must be a string");
...
function parseHeader(value: unknown): JsonRecord {
  if (!isRecord(value)) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 artifact header must be a JSON object");
  ...
  if (Object.keys(value).some((key) => !allowedKeys.has(key))) {
    fail("RECOVERY_FORMAT_UNSUPPORTED", "Alpha2 SessionHeader contains fields outside the audited Alpha2 format");
  }
  if (value.type !== "session") fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 artifact does not start with a session header");
  if (value.version !== 0) fail("RECOVERY_FORMAT_UNSUPPORTED", `Unsupported Alpha2 session format version ${String(value.version)}`);
  if (typeof value.id !== "string" || value.id.length === 0
    ...
    fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 artifact contains an invalid SessionHeader");
  }
  if ((value.cwd !== undefined && typeof value.cwd !== "string")
    ...
    fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 artifact contains invalid optional SessionHeader fields");
  }
  return value;
}

function nativeHeader(storageHeader: JsonRecord): JsonRecord {
  const { type: _type, ...header } = storageHeader;
  return header;
}
...
function decodeStorageRecord(value: unknown): readonly JsonValue[] {
  if (!isRecord(value)) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 storage record must be a JSON object");
  let expanded: JsonRecord = value;
  if (Object.hasOwn(value, "sourceEventSeqs")) {
    if (!Number.isSafeInteger(value.seq) || Number(value.seq) < 0) {
      fail("RECOVERY_ARTIFACT_CORRUPT", "Stored Alpha2 event has invalid seq provenance");
    }
...
function validateEvent(value: JsonValue, expectedSeq: number): void {
  if (!isRecord(value) || typeof value.type !== "string" || !Number.isSafeInteger(value.seq)
    ...
    fail("RECOVERY_ARTIFACT_CORRUPT", `Alpha2 event ${expectedSeq} has an invalid envelope`);
  }
  if (value.seq !== expectedSeq) {
    fail("RECOVERY_SEQUENCE_GAP", `Alpha2 event sequence gap: expected ${expectedSeq}, got ${String(value.seq)}`);
  }
}
...
function scanZstdFrames(buffer: Buffer): readonly { readonly start: number; readonly end: number }[] {
  ...
  while (offset < buffer.length) {
    ...
    if (!checksum) fail("RECOVERY_FORMAT_UNSUPPORTED", "Alpha2 Zstandard frames must carry the audited checksum flag");
...
function decodeArtifactBytes(bytes: Buffer, compression: RuntimeArtifact["compression"]): Buffer {
  ...
  if (frames.length === 0) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 Zstandard artifact has no complete header frame");
  ...
  for (const frame of frames) {
    ...
    try {
      ...
      fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 Zstandard frame failed checksum or decompression", error);
    }
    total += output.length;
    if (total > MAX_DECOMPRESSED_BYTES) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 artifact exceeds decompression limit");
    if (output.length === 0 || output.at(-1) !== 10) {
      fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 Zstandard frame does not end at a JSONL record boundary");
    }
    if (decoded.length === 0 && output.indexOf(10) !== output.length - 1) {
      fail("RECOVERY_FORMAT_UNSUPPORTED", "Alpha2 Zstandard header frame must contain exactly one SessionHeader row");
    }
    decoded.push(output);
  }
...
function decodeArtifactJsonl(bytes: Buffer): DecodedRuntimeArtifact {
  ...
  if (completeEnd < 0) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 artifact has no complete header line");
  ...
  for (let newline = complete.indexOf(10, start); newline !== -1; newline = complete.indexOf(10, start)) {
    if (newline - start > MAX_JSONL_LINE_BYTES) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 JSONL line exceeds size limit");
    lines.push(complete.subarray(start, newline));
    if (lines.length > MAX_JSONL_LINES) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 JSONL artifact exceeds line limit");
    start = newline + 1;
  }
  const headerLine = lines[0];
  if (headerLine === undefined || headerLine.length === 0) fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 artifact is header-less");
  let parsedHeader: unknown;
  try {
    parsedHeader = JSON.parse(headerLine.toString("utf8"));
  } catch (error) {
    fail("RECOVERY_ARTIFACT_CORRUPT", "Alpha2 session header is not valid JSON", error);
  }
  const header = parseHeader(parsedHeader);
  const events: JsonValue[] = [];
  for (let index = 1; index < lines.length; index += 1) {
    const line = lines[index]!;
    if (line.length === 0) fail("RECOVERY_ARTIFACT_CORRUPT", `Alpha2 artifact contains an empty event row at line ${index + 1}`);
    let parsed: unknown;
    try {
      parsed = JSON.parse(line.toString("utf8"));
    } catch (error) {
      fail("RECOVERY_ARTIFACT_CORRUPT", `Alpha2 event row ${index} is not valid JSON`, error);
    }
...
async function enumerateArtifacts(root: string): Promise<readonly RuntimeArtifact[]> {
  ...
  for (const projectEntry of projectEntries.sort((left, right) => left.name.localeCompare(right.name))) {
    if (!projectEntry.isDirectory() || projectEntry.isSymbolicLink()) {
      fail("RECOVERY_LAYOUT_UNSUPPORTED", `Unexpected entry in Alpha2 persistence root: ${projectEntry.name}`);
    }
    ...
    for (const sessionEntry of sessionEntries.sort((left, right) => left.name.localeCompare(right.name))) {
      if (!sessionEntry.isDirectory() || sessionEntry.isSymbolicLink()) {
        fail("RECOVERY_LAYOUT_UNSUPPORTED", `Unexpected Alpha2 project entry: ${projectEntry.name}/${sessionEntry.name}`);
      }
      const directory = join(project, sessionEntry.name);
      const entries = await readdir(directory, { withFileTypes: true });
      if (entries.some((entry) => !entry.isFile() || entry.isSymbolicLink())) {
        fail("RECOVERY_LAYOUT_UNSUPPORTED", `Alpha2 session directory contains a non-file entry: ${sessionEntry.name}`);
      }
      ...
      if ((zstd === undefined) === (plain === undefined) || entries.length !== 1) {
        fail("RECOVERY_LAYOUT_UNSUPPORTED", `Alpha2 session directory must contain exactly one supported artifact: ${sessionEntry.name}`);
      }
      const compression = zstd === undefined ? "none" : "zstd";
      if (rootCompression !== undefined && rootCompression !== compression) {
        fail("RECOVERY_LAYOUT_UNSUPPORTED", "Alpha2 persistence root mixes plaintext and Zstandard artifacts");
      }
...
async function decodeArtifact(artifact: RuntimeArtifact): Promise<DecodedRuntimeArtifact> {
  ...
  if (!metadata.isFile() || metadata.size > MAX_ARTIFACT_BYTES) {
    fail("RECOVERY_ARTIFACT_CORRUPT", `Alpha2 artifact is not a bounded regular file: ${artifact.path}`);
  }
...
/**
 * Recover only the durable, contiguous Alpha2 tail that reached the official
 ...
 */
export async function recoverAlpha2RuntimeTail(
  input: Alpha2RuntimeTailRecoveryInput,
): Promise<readonly NativeAppendOperation[]> {
  if (!isAbsolute(input.persistenceRoot)) fail("RECOVERY_ROOT_INVALID", "Alpha2 recovery root must be absolute");
  const root = await realpath(resolve(input.persistenceRoot)).catch((error: unknown) => fail(
    "RECOVERY_ROOT_INVALID",
    "Alpha2 recovery root does not exist",
    error,
  ));
  if (!(await stat(root)).isDirectory()) fail("RECOVERY_ROOT_INVALID", "Alpha2 recovery root must be a directory");
  ...
  for (const artifact of await enumerateArtifacts(root)) {
    ...
    if (decoded.events.length < mapping.nativeRevision) {
      fail("RECOVERY_PREFIX_REWRITTEN", `Alpha2 artifact lost committed events for ${nativeId}`);
    }
    for (let seq = 0; seq < mapping.nativeRevision; seq += 1) {
      if (!sameJson(decoded.events[seq]!, mapping.committedEvents[seq]!)) {
        fail("RECOVERY_PREFIX_REWRITTEN", `Alpha2 committed prefix differs at ${nativeId} seq ${seq}`);
      }
    }

...
export const pluginDataRecordSchema = z.strictObject({ namespace: z.string().min(1).max(128), dataType: z.string().min(1).max(256),
  recordId: z.string().min(1).max(1024), value: jsonValueSchema });
...
export interface PluginDataMappingAdapter {
  ...
  withAccess?<T>(targets: readonly PluginDataTarget[], work: () => Promise<T>): Promise<T>;
  /** Capture only owned, available data. Missing members never mean deletion. */

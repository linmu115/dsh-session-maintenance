...
import { isV3PreparationEvents, isV3PreparationArtifact } from "./native-session-codec.js";
...
export async function recoverV3RuntimeTail(input:{readonly runId:RunId;readonly persistenceRoot:string;readonly sessions:readonly V3CommittedRuntimeSession[];readonly observedAt:string;readonly onIgnoredPreparationArtifact?:(id:NativeSessionId)=>void|Promise<void>}):Promise<readonly NativeAppendOperation[]> {
 ...
 const artifacts=await inspectV3NativeSpace(input.persistenceRoot),registered=artifacts.filter(a=>mappings.has(a.nativeSessionId));
 for(const artifact of artifacts){seen.add(artifact.nativeSessionId);const mapping=mappings.get(artifact.nativeSessionId);if(!mapping){if(isV3PreparationArtifact(artifact,registered)){await input.onIgnoredPreparationArtifact?.(artifact.nativeSessionId);continue;}throw new TypeError(`Unmapped native history ${artifact.nativeSessionId}`);}

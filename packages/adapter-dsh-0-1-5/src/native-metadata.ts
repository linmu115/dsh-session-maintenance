...
export function ownsV3CanonicalEvent(event:CanonicalEventV1):boolean {return event.extensions.adapterId===currentManifest().id||event.id.startsWith(`${currentManifest().id}:`);}
export async function restoreV3Metadata(events:readonly CanonicalEventV1[],evidence:Pick<AdapterEvidencePort,"readEvidence">):Promise<readonly CanonicalEventV1[]> {
 return Promise.all(events.map(async event=>{
  ...
  const stored=await evidence.readEvidence(ref as AdapterEvidenceRef,currentManifest().id);
  if(!stored||stored.adapterId!==currentManifest().id||stored.nativeFormatId!==currentFormatId()||stored.sourceKind!==content.sourceKind)throw new TypeError("V3 source evidence is missing or has wrong owner/format");

...
import { record, digest, FORMAT_ID } from "./common.js";
import { manifest } from "./manifest.js";
import { assertInformational } from "./references-remap.js";
import { validateV3Events } from "./official.js";
export function ownsV3CanonicalEvent(event:CanonicalEventV1):boolean {return event.extensions.adapterId===manifest.id||event.id.startsWith(`${manifest.id}:`);}
export async function restoreV3Metadata(events:readonly CanonicalEventV1[],evidence:Pick<AdapterEvidencePort,"readEvidence">):Promise<readonly CanonicalEventV1[]> {
 return Promise.all(events.map(async event=>{
  ...
  const stored=await evidence.readEvidence(ref as AdapterEvidenceRef,manifest.id);
  if(!stored||stored.adapterId!==manifest.id||stored.nativeFormatId!==FORMAT_ID||stored.sourceKind!==content.sourceKind)throw new TypeError("V3 source evidence is missing or has wrong owner/format");

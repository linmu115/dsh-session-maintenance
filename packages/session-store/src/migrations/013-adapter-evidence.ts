/**
 * Persists Adapter-private evidence by content reference and exposes the
 * adapter.evidence runtime breakpoint without placing native payloads in MCSF.
 */
export const MIGRATION_013 = `
PRAGMA defer_foreign_keys = ON;

CREATE TABLE adapter_evidence (
  evidence_ref TEXT PRIMARY KEY,
  adapter_id TEXT NOT NULL,
  native_format_id TEXT NOT NULL,
  source_kind TEXT NOT NULL,
  object_id TEXT NOT NULL,
  byte_length INTEGER NOT NULL CHECK (byte_length >= 0),
  observed_at TEXT NOT NULL,
  created_at TEXT NOT NULL,
  UNIQUE (adapter_id, object_id)
) STRICT;

CREATE INDEX adapter_evidence_object_idx
  ON adapter_evidence(object_id);
CREATE INDEX adapter_evidence_adapter_created_idx
  ON adapter_evidence(adapter_id, created_at);
...
ALTER TABLE run_status_events RENAME TO run_status_events_v12;
...
SELECT
  ...
  parent_event_id, span_id, error_code, duration_ms, diagnostic_detail_ref,
  event_json
FROM run_status_events_v12
ORDER BY sequence;

DROP TABLE run_status_events_v12;

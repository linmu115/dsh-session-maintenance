...
describe("retention schema 21 compatibility boundary", () => {
  it.each([21,22,23])("reads schema %s body references and verifies a static recovery point without changing it", async (schema) => {
    const f = await retentionFixture();
    try {
      const body = await f.addVersion("head", "retained schema 21 body");
      expect(f.database.prepare("SELECT MAX(version) version FROM schema_migrations").get()?.version).toBe(23);
      const path = await f.external("schema21");
      if(schema<23){const db=new DatabaseSync(path);try{db.exec("DROP TABLE context_read_executions; DELETE FROM schema_migrations WHERE version=23");if(schema===21)db.exec("DROP TABLE extension_conflicts; DROP TABLE extension_objects; DROP TABLE extension_connections; DELETE FROM schema_migrations WHERE version=22");}finally{db.close();}}
  ...
  it.each([15, 18, 24, 999])("continues to reject unsupported schema %s for references and static candidates", async (schema) => {
    const f = await retentionFixture();
    try {

...
it("SM-08 migration failure rolls back registry and registration, and retry is idempotent",async()=>{
  ...
  try {
    ...
    db.exec("DROP TABLE context_read_executions; DROP TABLE extension_conflicts; DROP TABLE extension_objects; DROP TABLE extension_connections; DROP TABLE codex_project_mapping_removals; DROP TABLE codex_project_mapping_policy; DROP TABLE retention_batches; DROP TABLE retention_resources; DROP TABLE retention_sources; DROP TABLE retention_roots; DELETE FROM schema_migrations WHERE version >= 19; CREATE TABLE retention_sources(synthetic_conflict TEXT)"); db.close();
...
it("SM-09 journal migration preserves existing registry on failure and can retry",async()=>{
  ...
  try {
    let db=openMaintenanceDatabase(path);db.exec("DROP TABLE context_read_executions; DROP TABLE extension_conflicts; DROP TABLE extension_objects; DROP TABLE extension_connections; DROP TABLE codex_project_mapping_removals; DROP TABLE codex_project_mapping_policy; DELETE FROM schema_migrations WHERE version>=20; DROP TABLE retention_batches; CREATE TABLE retention_batches(synthetic_conflict TEXT)");db.close();

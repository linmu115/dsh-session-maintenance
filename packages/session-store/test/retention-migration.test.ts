...
it("SM-08 migration failure rolls back registry and registration, and retry is idempotent",async()=>{
  ...
  try {
    ...
    db.exec("DROP TABLE retention_batches; DROP TABLE retention_resources; DROP TABLE retention_sources; DROP TABLE retention_roots; DELETE FROM schema_migrations WHERE version >= 19; CREATE TABLE retention_sources(synthetic_conflict TEXT)"); db.close();
...
it("SM-09 journal migration preserves existing registry on failure and can retry",async()=>{
  ...
  try {
    let db=openMaintenanceDatabase(path);db.exec("DELETE FROM schema_migrations WHERE version=20; DROP TABLE retention_batches; CREATE TABLE retention_batches(synthetic_conflict TEXT)");db.close();

...
import type { EndpointRecoveryQuery } from '@linmu/dsh-session-contracts';
...
export interface IntegrationServiceOptions {
  ...
  readonly recoveryRuns: EndpointRecoveryQuery;
}
    ...
    if (action === "disconnect") {
      ...
      await this.options.writes.run("integration-disconnect", async () => {
        ...
        if (current?.kind === 'dsh' && current.profileId !== null) {
          const runs = await this.options.recoveryRuns(this.options.stateRoot, current.instanceId, current.profileId);

...
export function canonicalEventsFor(input: {
  ...
  return input.artifact.events.map((event, index) => {
    ...
    return {
      ...
      kind: shape.kind,
      role: shape.role,
      content: (event as { data?: unknown }).data ?? {},
      contentDigest: `sha256:${createHash('sha256').update(JSON.stringify(event)).digest('hex')}`,
      rawPayload: event,
      extensions: { nativeFormatVersion: 3, dshEventType: dataType, adapterId: manifest.id,
        ...(owner ? { extensionNamespace: owner, extensionDataType: dataType } : {}),

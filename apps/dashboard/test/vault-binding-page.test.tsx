...
it("shows registered instances without a running provider and manages bindings in a modal",async()=>{
 ...
 const click=async(text:string)=>{await act(async()=>{[...node.querySelectorAll<HTMLButtonElement>("button")].find(button=>(button.getAttribute("aria-label") ?? button.textContent)===text)!.click();});};
 try {
  await act(async()=>root.render(<VaultBindingPage api={api}/>)); expect(api.listManagedVaults).not.toHaveBeenCalled();
  await click("离线实例 · web，管理 Vault 绑定"); expect(node.querySelector('dialog')?.hasAttribute("open")).toBe(true); expect(node.textContent).toContain("Vault A");

...
const errorText = (error: unknown) => error instanceof Error && /HTTP 431/u.test(error.message)
  ? "浏览器请求信息过大，目录暂时无法加载（HTTP 431）。这与 DSH 实例是否启动无关。"
  : error instanceof Error ? error.message : "扩展目录暂时无法读取";
type DirectoryStatus = "loading" | "ready" | "empty" | "error";
...
export function ExtensionBusinessDirectory({ api, onOpenSession, renderDetail, hideAdapterNavigation = false }: {
  ...
  const directoryKey = `${scope}:${panel?.adapterId}:${deleted}:${refresh}`;
  const [directoryState, setDirectoryState] = useState<{key: string; status: DirectoryStatus}>();
  const directoryStatus = directoryState?.key === directoryKey ? directoryState.status : "loading";
  const onDirectoryStatus = useCallback((status: DirectoryStatus) => setDirectoryState({key: directoryKey, status}), [directoryKey]);
    ...
    {panels && panels.length > 0 ? <>
      ...
      {panel && base ? <section role="tabpanel" aria-label={panel.label}>
        ...
        <div className="extension-directory-layout">
          <section className="dsm-surface extension-directory-tree"><DirectoryLevel key={directoryKey} onStatus={onDirectoryStatus} api={api} query={{...base, level: "workspaces"}} selected={selected} onSelect={setSelected} onOpenSession={onOpenSession}/></section>
          <section className="dsm-surface extension-directory-detail">
            ...
            </> : directoryStatus === "error" ? <EmptyState title="工作区目录加载失败" description="请在左侧重试目录加载，或使用上方刷新按钮。已保存的扩展数据不会因此丢失。"/>
              : directoryStatus === "loading" ? <LoadingState label="正在读取工作区…"/>
              : directoryStatus === "empty" ? <EmptyState title="当前范围暂无扩展条目" description="可切换实例或条目状态，查看已保存的扩展数据。"/>
...
function DirectoryLevel({api, query, selected, onSelect, onOpenSession, onStatus}: {
  api: ExtensionBusinessApi; query: ExtensionDirectoryQuery; selected?: ExtensionDirectoryObject | undefined; onStatus?(status: DirectoryStatus): void;
  ...
  const [error, setError] = useState<string>();
  useEffect(() => {
    const controller = new AbortController(); setLoading(true); setError(undefined); onStatus?.("loading");
    void api.listExtensionDirectory({...JSON.parse(key) as ExtensionDirectoryQuery, limit: 30, ...(cursor ? {after: cursor} : {})}, controller.signal).then(page => {
      ...
      setNextCursor(page.nextCursor); setLoading(false); onStatus?.(page.items.length || cursor ? "ready" : "empty");
    }, failure => { if (!controller.signal.aborted) { setError(errorText(failure)); setLoading(false); onStatus?.("error"); } });
    return () => controller.abort();
  }, [api, key, cursor, retry, onStatus]);
  return <div className={`extension-tree-level extension-tree-${query.level}`} aria-label={query.level === "workspaces" ? "扩展工作区目录" : query.level === "sessions" ? "所属会话目录" : "会话扩展条目"}>
    ...
    {error ? <div className="extension-directory-empty"><p role="alert">{error}</p><Button onClick={() => setRetry(value => value + 1)}>重试加载</Button></div> : null}

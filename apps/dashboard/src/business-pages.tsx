...
function BusinessAction({ api, page, action }: { api: BusinessPagesApi; page: BusinessPage; action: BusinessPageActionDescriptor }) {
  ...
  return <form onSubmit={event => { event.preventDefault(); void submit(); }}>
    <fieldset disabled={busy || waiting || !page.online}><legend>{action.label}</legend>
      {action.fields.map(field => <label className={field.kind === "boolean" ? "sync-workspace" : "field"} key={field.id}>
        {field.label}{field.required && field.kind !== "boolean" ? "（必填）" : ""}
    ...
    {error ? <p role="alert">{error}{attempt.current ? "；再次提交会沿用同一操作编号和原始输入核对回执。" : ""}</p> : null}
...
export function BusinessPages({ api, renderDirectory }: { api: BusinessPagesApi; renderDirectory(owner: BusinessPageOwner, adapterId: string): ReactNode }) {
  ...
  return <div className="page-stack"><div className="dsm-action-row"><h2>插件信息页</h2><Button onClick={() => setRevision(value => value + 1)}>刷新信息页</Button></div>
    {error ? <p role="alert">{error}</p> : !pages ? <LoadingState label="正在读取插件信息页…" /> : pages.length === 0 ? <p className="muted">插件注册业务信息页后，会在这里显示接入状态、绑定操作和数据目录。</p> : null}
    {pages?.map(page => {
      const pageKey = JSON.stringify(page.owner);
      return <Surface key={pageKey} title={page.snapshot.title}><div className="surface-body"><p>{page.owner.instanceId} · {page.owner.profileId} <Badge tone={page.online ? "success" : "neutral"}>{page.online ? "提供方在线" : "提供方离线，保留最后信息"}</Badge></p>
        {page.snapshot.sections.map(section => <section key={section.id}><h3>{section.title}</h3>
          {section.kind === "summary" ? <p>{section.text}</p> : section.kind === "status" ? <p><Badge tone={section.state === "ready" ? "success" : section.state === "warning" ? "warning" : "neutral"}>{section.label}</Badge></p>
            : section.kind === "key-values" ? <dl>{section.items.map((item, index) => <div key={index}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl>
            : section.kind === "data-directory" ? <><Button onClick={() => setDirectory(directory === `${pageKey}:${section.id}` ? undefined : `${pageKey}:${section.id}`)}>查看数据目录</Button>{directory === `${pageKey}:${section.id}` ? renderDirectory(page.owner, section.adapterId) : null}</>
            : section.actions.map(action => <BusinessAction key={`${action.id}:${action.expectedRevision}`} api={api} page={page} action={action} />)}
        </section>)}
      </div></Surface>;
    })}
  </div>;

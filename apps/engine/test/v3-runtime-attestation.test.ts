...
it("pins RC2 scope, closure, capabilities and exact artifact bytes before granting a binding",async()=>{
 const root=await mkdtemp(join(tmpdir(),"synthetic-runtime-attestation-"));try{
  ...
  for(const engineVersion of ["0.1.33-rc2.39","0.1.33-rc2.40"]){await save({...receipt,engineVersion});expect((await verifyDsh015RuntimeAttestation(input)).coreBinding.path).toBe(files.at(-1)!.path);}

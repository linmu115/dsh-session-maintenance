...
describe("Maintenance external lifecycle provider", () => {
  it("refuses an offline managed launch without starting Engine or waiting for a child", async () => {
    const stateRoot = await mkdtemp(join(tmpdir(), "SYNTHETIC-engine-start-failed-"));
    cleanups.push(() => rm(stateRoot, { recursive: true, force: true }));
    const dispose = vi.fn();
    const sleep = vi.fn(async () => {});
    const startEngine = vi.fn(async () => ({ failure: async () => "WRITER_OWNER_UNKNOWN", dispose }));
    const provider = new MaintenanceExternalLifecycleProvider(stateRoot, {
      startEngine,
      sleep,
    });
    await expect(provider.handle(prepareRequest())).rejects.toThrow("请先启动并确认维护服务就绪");

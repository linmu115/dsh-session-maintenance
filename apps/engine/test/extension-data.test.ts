...
import { openMaintenanceDatabase, SqliteExtensionRepository, MAINTENANCE_SCHEMA_VERSION } from "@linmu/dsh-session-store";
...
describe("pluggable extension data",()=>{
  it("connects all six namespaces in the current graph session names cohort",async()=>{
    ...
    expect(service.panels()).toHaveLength(6);
    service.connect({...connect,plugins:currentPlugins.map(plugin=>({...plugin,pluginVersion:"99.0.0"}))});
    expect(service.panels().every(panel=>panel.status==="incompatible")).toBe(true);
  });
  it("round trips two actual plugin shapes and cross-session references without touching session tables",async()=>{
    ...
    expect(f.db.prepare("SELECT MAX(version) v FROM schema_migrations").get()?.v).toBe(MAINTENANCE_SCHEMA_VERSION);
  });

...
export class CodexProjectMappingService {
  ...
  private learningSources(instanceId: string): Set<string> {
    return new Set((this.options.database.prepare("SELECT codex_thread_id FROM learning_bindings WHERE codex_instance_id=?").all(instanceId) as { codex_thread_id: string }[]).map(row => row.codex_thread_id));
  }
  ...
  async get(): Promise<CodexProjectMappingConfiguration> {
    ...
    for (const result of results) {
      ...
      const learning = this.learningSources(instance.id);
      ...
      for (const project of directory.projects) projects.push({ key: codexProjectKey(instance.id, project.projectId), instanceId: instance.id,
        ...
        sessionCount: project.memberThreadIds.filter(id => !learning.has(id)).length,
        learningManagedCount: project.memberThreadIds.filter(id => learning.has(id)).length,
        eligible: directory.safeForSelection, issues: [...directory.issues] });
    }
  ...
  async readScope(instance: RegisteredInstance, saved?: CodexProjectMappingPolicy) {
    ...
    return { revision: saved === undefined ? policy.activeRevision : policy.revision,
      projectIds: relevant.map(key => available.get(key)!), directory: { ...directory,
        assignments: Object.fromEntries(Object.entries(directory.assignments).filter(([id]) => !learning.has(id))),
        projects: directory.projects.map(project => ({ ...project, memberThreadIds: project.memberThreadIds.filter(id => !learning.has(id)) })),
      } };
  }
  ...
  async activateForStartup(sync: (policy: CodexProjectMappingPolicy) => Promise<void>) {
    return this.options.writes.run("codex-project-mapping-activate", async () => {
      ...
      const learning = new Map(snapshots.map(({ instance }) => [instance.id, this.learningSources(instance.id)]));
      for (const { instance, directory } of snapshots) for (const project of directory.projects) {
        ...
        for (const sessionId of project.memberThreadIds) {
          if (learning.get(instance.id)?.has(sessionId)) continue;

import { manifest as gptManifest, FORMAT_ID as GPT_FORMAT, CAPABILITY as GPT_CAPABILITY } from "@linmu/dsh-session-adapter-gpt-compat";
...
const schema=z.strictObject({schemaVersion:z.literal(1),instanceId:z.string(),profileId:z.string(),homeRoot:z.string(),adapterId:z.enum(["dsh-0.1.5","dsh-gpt-compat"]),formatId:z.enum(["dsh-0.1.5-v3-jsonl-zstd-v1","dsh-gpt-compat-v1-jsonl-zstd"]),runtimeVersion:z.literal("0.1.5-rc.2"),engineVersion:z.enum(["0.1.32-rc2.2","0.1.33-rc2.1","0.1.33-rc2.2","0.1.33-rc2.3","0.1.33-rc2.4","0.1.33-rc2.5","0.1.33-rc2.6","0.1.33-rc2.7","0.1.33-rc2.8","0.1.33-rc2.9","0.1.33-rc2.10","0.1.33-rc2.11","0.1.33-rc2.12","0.1.33-rc2.13","0.1.33-rc2.14","0.1.33-rc2.15","0.1.33-rc2.16","0.1.33-rc2.17","0.1.33-rc2.18","0.1.33-rc2.19","0.1.33-rc2.20","0.1.33-rc2.21","0.1.33-rc2.22","0.1.33-rc2.23","0.1.33-rc2.24","0.1.33-rc2.25","0.1.33-rc2.26","0.1.33-rc2.27","0.1.33-rc2.28","0.1.33-rc2.29"]),launcherCapabilityDigest:z.string().regex(/^[a-f0-9]{64}$/u),runtimeCapabilities:z.array(z.string()),files:z.array(fileSchema)});
...
export async function verifyDsh015RuntimeAttestation(input:{profileRoot:string;instanceId:string;profileId:string;homeRoot:string;cliPath:string;launcherDigest:string|null;resolvedManifests:readonly string[];expectedAdapterId?:string}):Promise<{runtimeCapabilities:readonly string[];digest:string;coreBinding:{path:string;sha256:string}}> {
 ...
 if (receipt.adapterId !== (input.expectedAdapterId ?? "dsh-0.1.5") || receipt.formatId !== (receipt.adapterId === gptManifest.id ? GPT_FORMAT : "dsh-0.1.5-v3-jsonl-zstd-v1")) throw new IntegrationError("FORMAT_ADAPTER_MISMATCH","会话格式必须绑定独立且匹配的 Adapter。");
 if (receipt.adapterId === gptManifest.id && (!receipt.runtimeCapabilities.includes(GPT_CAPABILITY) || !receipt.files.some(f=>f.role==="sessionFormatPlugin"))) throw new IntegrationError("GPT_FORMAT_ATTESTATION_REQUIRED","缺少 GPT 插件格式能力及构件验证。");

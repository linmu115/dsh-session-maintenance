...
function compatiblePlugin(version: string | null): boolean {
  return ["0.2.19","0.2.20","0.2.24","0.2.25-rc2.1","0.2.25-rc2.2"].includes(version ?? "");
}
  ...
  if (raw !== undefined) {
    ...
    for (const instance of catalog.instances) {
      ...
      for (const entry of entries) {
        ...
        const pluginReady = (version.version === "0.1.5-rc.2" ? pluginVersion === "0.2.25-rc2.2" : compatiblePlugin(pluginVersion)) && plugin !== null && pluginRuntime?.path === plugin.path && maintenanceIntegrationBundleReady(plugin.patches) && Array.isArray(bundles) && bundles.includes("dsh-session-maintenance");
        const extraBundles = [];

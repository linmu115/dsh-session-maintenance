...
它不启动 DSH 实例，不修改开机自启，不删除会话或锁文件，也不自动回收尚未确认退出的 DSH 运行。已安装匹配的启动恢复补丁时，Launcher 会在启动 DSH 前自动检查并正式恢复有退出证明的旧运行；缺少补丁、退出证据不明或恢复失败时仍需处理具体原因。启动失败时保留日志路径，日志位于 Maintenance 状态目录的 `logs` 下。

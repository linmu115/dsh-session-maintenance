[CmdletBinding()]
param(
    [string]$InstallationConfig = (Join-Path $env:LOCALAPPDATA 'DSH-Session-Maintenance/engine-installation.json'),
...
# Maintenance owns its executable selection independently of Launcher.
$config = Get-Content -LiteralPath $InstallationConfig -Raw -Encoding UTF8 | ConvertFrom-Json
if ($config.schemaVersion -ne 1) { throw 'Unsupported Maintenance installation descriptor.' }
$entry = $config.entry
...
try {
    ...
    do {
        ...
        if ($null -ne $ready) {
            ...
            Write-Host 'The Maintenance engine is independent of Launcher; instance connection is verified after startup.'
            exit 0
        }
        $child.Refresh()

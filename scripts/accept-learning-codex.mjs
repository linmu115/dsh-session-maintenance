...
const historyMode=process.argv[2]??'legacy';assert.ok(['legacy','paginated'].includes(historyMode));
...
try {
  ...
  t=await transport();await t.request('thread/resume',{threadId:thread.id,excludeTurns:true});console.log('resumed');
  ...
  console.log(JSON.stringify({passed:true,root,version:'0.153.4',historyMode,persisted:true,reopened:true,contextInNextRequest:true,newMessages:collected.messages.length,realModelCalls:0}));

...
2026-09-19 图片策略更新：用户明确要求跳过图片，已安装 .50/.11。学习投影保留文字，纯图片用 `[图片已跳过]` 占位并显示数量，原图不变；未知非文本材料仍拒绝。CLI 0.153.4 legacy/paginated 均已通过隔离协议验收。下方只支持 legacy/拒绝图片为早期限制，现状见 [图片过滤报告](../../../reports/2026-09-19-learning-images.md)。真实“机试DeepLearning”尚未通过共同历史预检，未绑定。

...
status: current
date: 2026-09-23
summary: 重建阶段 62 条记录结构有效；三类图机械条件就绪，已在 HTML 实测节点与文档跳转；后续开发日志补录另见文末。
relations:
  - relation: verifies
...
2026-09-23 后续补录：用户要求把旧开发日志放回新地图，因此上文 62 条记录和旧资产去向数字只描述本次重建交付时点。补录的恢复范围、重新统计与检查见 [[JRN-20260923-history-restoration]] 和 [更新后的逐条去向](../../migration/README.md)；历史内容没有被重新验收为当前产品行为。

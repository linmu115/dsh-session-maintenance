...
2026-09-19 后续补记（本段来源索引待补，原 history 索引仅覆盖 09-17）：任务 `01a0b8bb-ae8e-7202-af8d-ed8d46b04487` 中，用户追问共同前缀不通过，追加双向维护必须自动解除普通同步的要求。对照普通导入发现有效历史实际上 49 条一致，差异来自压缩历史、批注封装和公开进度抽取；.51 部署并真实关联成功，原历史不变。用户随即反馈同步无反应，页面实际为零增量误触发整段历史预算检查；.52 修复并在真实页面完成仅保存回执的交接，原 Codex 日志未变化。用户另问是否跳转，对当前行为作出说明，没有扩展为自动跳转需求。详细版本、测试、失败记录及边界见 [本轮报告](../../../reports/2026-09-19-learning-prefix-and-sync.md)。

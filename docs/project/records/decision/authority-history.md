...
2026-09-21 补充：09-10 持久空间规格中「存在未解决恢复时拒绝覆盖原生目录」及验收项 N09 与用户新确认的「覆盖式同步」冲突，按用户注释不再适用。该规格是外部绑定来源，原文不改写，冲突登记在 [[DEC-directory-connect-sync-authority]]；规格其余部分（空间身份与所有权、启动准备事务、正常关闭与恢复保护）继续有效。

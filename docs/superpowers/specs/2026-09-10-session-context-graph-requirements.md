...
初稿：2026-09-10。需求确认：2026-09-14。实施更新：2026-09-15。状态：会话主干图、右键操作、蓝色引用标记、统一撤销和轻量读取记录已实现并安装到 DSH 0.1.5-rc.2 副本。自动检查、合成浏览器流程与副本启动检查已完成；真实模型回答和用户操作验收不由这些检查替代。详见[联合发布报告](../../reports/2026-09-15-session-main-graph-release.md)。

2026-09-15 新增阶段：LLM 管理自身主干、用户请求索引、活动披露窗口与原生上下文释放，详见[补充规格](2026-09-15-native-agent-context-management.md)和[实现与验证报告](../../changes/2026-09-15-native-context-management.md)。这部分已实现代码及原生联合验证，范围仅为 DSH 原生 Agent，托管引擎暂不纳入；与上述历史交付分批记录。

初稿文档基线：Session Maintenance 提交 `5fd467922dcc8ccda0f8d740fecd3811dc6ef2fb`，已包含 `fd6db7a` 的持久原生会话空间实现提交。该历史基线不代表后续交付状态；当前配套版本、数据面板和副本核查证据见联合发布报告。
...
- [原生 Agent 主干图、用户请求索引与上下文管理](2026-09-15-native-agent-context-management.md)
...
| D17 | LLM 通过 DSH 原生工具管理当前会话自己的主干节点、连接及披露窗口，UI 与工具使用同一后端操作 |
| D18 | 用户请求形成可分页的独立结构索引，以稳定消息身份定位问答/执行；上游索引同样受固定版本和截止范围约束 |
| D19 | 授权范围、活动窗口、当前输入保留集合分开；支持不连续片段，窗口变更不能越过授权上限 |
| D20 | 用完材料默认释放正文并保留连接；暂停/恢复与永久解除分别操作，真实释放从后续原生模型请求生效 |
| D21 | 提供来源状态、读取覆盖、保留标记、稳定定位和允许范围内的材料发现能力，继续保持存储轻量 |
| D22 | 新增自主上下文管理首期仅支持 DSH 0.1.5-rc.2 原生 Agent，托管引擎暂不处理；以补充规格 NCA01–NCA24 验收 |
...
新增原生 Agent 阶段在这些位置之外管理活动窗口和当前输入保留集合，详见[补充规格第 4–5 节](2026-09-15-native-agent-context-management.md)。过去实际返回的范围继续作为历史事实；释放材料不把“已返回”改成“从未读取”，授权上限也不随窗口改变。
...
以下为验收要求，逐项证据与未执行的真实场景见联合发布报告及验收映射。自动检查使用合成会话和标记的临时目录；未发起真实模型回答，副本交互验收由用户执行。
...
| AC41 | 右键来源选文的蓝色引用符号 | 提供删除引用；同一选文有多条引用时逐条选择。权威撤销成功后移除对应蓝标、停止上下文读取并刷新目标主干；保留普通红贴纸及真实会话 |
| AC42 | DSH 或 Maintenance 归档会话 | 撤销以该会话为来源或目标的活动引用，更新所有相关主干图；所属主干随会话归档。插件停用、画布关闭及退出尾部写入不影响此规则 |
| AC43 | 恢复已归档会话 | 仅恢复因所属会话归档的主干，已解除的引用不复活；手动删除的图不被恢复 |
| AC44 | 已有卡片开始会话，引用已发送但本地 Core 记录缺失 | 从目标会话范围内的权威记录恢复轻量读取授权，保留原来源版本和截止位置；不伪造历史提交、重新绑定或自动发送 |
| AC45 | 画布编辑中引用被删除或所属会话归档 | 收到通知或可见状态核对后合并最新关系，保留未保存布局与占位卡片；归档后转为只读，禁止保存/开始会话 |

2026-09-15 生命周期细化：来源蓝标删除以 Maintenance 的撤销确认为准；本地气泡尚未清理时明确提示待同步，不恢复已删除标记。仅收到明确的权威撤销状态才清理本地引用，离线或身份查找失败不能推断成删除。已发送引用恢复只保存固定来源的轻量索引，不复制会话正文、初始上下文或提交回执。

## 11. 本轮不做的功能

2026-09-14 的需求确认阶段仅交付文档；用户随后授权实施，2026-09-15 已更新 rc2 副本。本轮不批量删除真实图数据，不自动启动或迁移其它运行实例；不修改 DSH 官方源码作为默认接入方式；不重新引入全局维护网络/影响查看/批量准备重答；不新建逐轮上下文备份系统；不引入 EAC 或 `dsh-codex-session-sync` 运行兼容。
...
4. 已确认图关系默认可用于后续轮次，直到明确解除或来源不可用；管理入口为节点/边右键，读取仍受固定范围与每轮预算约束。草稿取消与已发送关系解除必须区分。
   新确认的原生 Agent 阶段增加模型工具与暂停/恢复、活动窗口和材料释放；详见补充规格。暂停不撤销授权，解除后不能通过恢复窗口复活。此项为新增待实现行为。
